...
def list_reports():
    ...
    return render_template("reports/list.html", reports=reports, reporter=reporter)
...
def create_report():
    ...
    if form.validate_on_submit():
        ...
        if form.image.data:
            ...
            image_filename = datetime.utcnow().strftime("%Y%m%d%H%M%S_") + filename  # unique filename
        ...
        report_data = {
            ...
            "date_reported": datetime.utcnow(),  # Auto-generate timestamp
...
def delete_report(report_id):
    ...
    db = current_app.db
    report = CrimeReport.get_report_by_id(report_id, db)
    if report and report["reported_by"] != ObjectId(current_user.get_id()):
